...
parser.add_argument('--dim', type=int, default=3 + 1) # dimension of the problem, e.g., one dim in time, and 100 dim in x
parser.add_argument('--mu', type=float, default=1) # mu in the equation
parser.add_argument('--dataset', type=str, default="Linear_Heat")
...
if args.dim == 11: args.trans = 2500
elif args.dim == 101: args.trans = 1500
elif args.dim == 1001: args.trans = 6000
elif args.dim == 10001: args.trans = 6000
print(args)
device = torch.device(args.device)
torch.manual_seed(args.SEED)
torch.cuda.manual_seed(args.SEED)
np.random.seed(args.SEED)
assert args.dataset == "Linear_Heat"
if args.dtype == "float64": torch.set_default_dtype(torch.float64)

def load_data_Linear_Heat(d):
    args.input_dim = d
    args.output_dim = 1
    def u(x, t):
    ...
    xf = np.random.randn(N_f, d - 1) * np.sqrt(2 - tf) + tf
    xf = np.concatenate([xf, tf], axis=-1)
    ff = np.zeros(N_f)

    t = np.random.rand(N_test, 1)
    x = np.random.randn(N_test, d - 1) * np.sqrt(2 - t) + t
    x = np.concatenate([x, t], axis=-1)
    u = u(x[:, :-1], x[:, -1:])
    return x, u, xf, ff

x, u, xf, ff = load_data_Linear_Heat(d=args.dim)
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # X, t = x[:, :-1], x[:, -1:]
        # return (args.dim - 1) / args.dim * (t + 1) - 2 * torch.sum(X, dim=1, keepdims=True) / args.dim
        return self.nn(x)
...
class PINN:
    def __init__(self):
        ...
        self.dim = args.dim
        self.dim_ = args.dim # constant for rescaling the network
    ...
    def Resample(self): # sample random points at the begining of each iteration
        ...
        xf = torch.randn(args.N_f, args.dim - 1) * torch.sqrt(2 - tf) + tf
        self.xf = torch.cat([xf, tf], dim=-1).to(device).requires_grad_()
        return

    def Linear_Heat(self):
        ...
        residual_pred = u_t.reshape(-1) - 0.5 * torch.sum(u_xx, dim=1) + args.mu * torch.sum(u_x, dim=1)
...
if args.save_loss:
    ...
    # filename = "saved_loss_l2/" + args.dataset + "_dim=" + str(args.dim) + "_method=" + str(args.method) + \
    #         "_SEED=" + str(args.SEED) + \
    #         "_dtype=" + args.dtype + "_x_std=" + str(args.x_std) + ".xlsx"
    # if args.method == 99:
    #     filename = "saved_loss_l2/" + args.dataset + "_dim=" + str(args.dim) + "_method=" + str(args.method) + \
    #         "_SEED=" + str(args.SEED) + "_trans=" + str(args.trans) + \
    #         "_dtype=" + args.dtype + "_x_std=" + str(args.x_std) + ".xlsx"
