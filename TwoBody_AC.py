...
parser.add_argument('--dataset', type=str, default="TwoBodyv2_AC")
...
if args.dim == 1000: args.trans = 8000
...
assert args.dataset == "TwoBodyv2_AC"

c = np.random.randn(1, args.dim - 1)
const_2 = 1
def load_data_TwoBody_Allen_Cahn(d):
    args.input_dim = d
    args.output_dim = 1
    def func_u(x):
...
x, u = load_data_TwoBody_Allen_Cahn(d=args.dim)
...
class PINN:
    ...
    def Resample(self): # sample random points at the begining of each iteration
        ...
        self.ff = torch.tensor(ff, dtype=torch.float32, requires_grad=True).to(device).reshape(-1, 1)
        return

    def TwoBodyv2_AC(self):
        x = self.xf
        u, u_x, u_xx = self.grad_estimator.dE(x)
        u_xx = -2 * u  - 4 * x * u_x + (1 - torch.sum(x**2, 1, True)) * u_xx
        u = (1 - torch.sum(x**2, 1, True)) * u
        residual_pred = torch.sum(u_xx, 1, True) + u - u**3 - self.ff
        return residual_pred
    
    def TwoBodyv2_AC2(self):
        x = self.xf
        u, u_x, u_xx = self.grad_estimator.dE(x)
        u_xx = -2 * u  - 4 * x * u_x + (1 - torch.sum(x**2, 1, True)) * u_xx
        u = (1 - torch.sum(x**2, 1, True)) * u
        u_2, u_3 = self.grad_estimator(x), self.grad_estimator(x)
        u_2, u_3 = (1 - torch.sum(x**2, 1, True)) * u_2, (1 - torch.sum(x**2, 1, True)) * u_3
        residual_pred = torch.sum(u_xx, 1, True) + u - u*u_2*u_3 - self.ff
        return residual_pred
    ...
    def get_loss_pinn_unbiased(self):
        f1 = self.TwoBodyv2_AC()
        f2 = self.TwoBodyv2_AC()
        mse_f = (f1 * f2).mean()
        saved_loss = f1.square().mean()
        return mse_f, saved_loss
    
    def get_loss_pinn_unbiased2(self):
        f1 = self.TwoBodyv2_AC2()
        f2 = self.TwoBodyv2_AC2()
        mse_f = (f1 * f2).mean()
        saved_loss = f1.square().mean()
        return mse_f, saved_loss
    ...
    def train_adam(self):
        ...
        for n in tqdm(range(self.epoch)):
            ...
            if args.method == 0:
                ...
                loss, saved_loss = self.get_loss_pinn_unbiased2()
            elif args.method == 99:
                if n <= args.trans:
                    loss, saved_loss = self.get_loss_pinn() 
                elif n > args.trans:
