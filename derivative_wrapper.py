...
class ImprovedSteinsWrapper(GaussianWrapper):
    def dE(self, X, sample_cnt=None):
        ...
        sample_u_0 = self.g(sample_X_0).reshape(sample_cnt, batch_size, 1)

        f = torch.mean(sample_u_plus, dim=0)

        df_dx = ((sample_u_plus-sample_u_minus)*e_X)/(2*self.x_std*self.x_std)
        df_dx = torch.mean(df_dx, dim=0)

        d2f = (e_X**2-self.x_std**2)*(sample_u_plus + sample_u_minus -2*sample_u_0)/(2*(self.x_std**4))
        d2f = torch.mean(d2f, dim=0)
        return f, df_dx, d2f
